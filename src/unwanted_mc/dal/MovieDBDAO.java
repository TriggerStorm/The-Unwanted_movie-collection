...
public class MovieDBDAO {
    ...
    public List<Movie> fetchIMBD() throws SQLException {
  //      List<String> imbdList = new Arraylist();
        List<Movie> movList = new ArrayList<>();
        List<Movie> matchingMov = new ArrayList<>();
        movList = fetchAllMovies();

        try ( Connection con = dbc.getConnection()) {
            String sql = "SELECT ratings.tconst, name.titleId, name.title, ratings.averageRating\n" + "FROM ratings INNER JOIN\n" + "name ON ratings.tconst = name.titleId";
            Statement stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(sql);
            for (int i = 0; i < movList.size(); i++) {
                Movie mov = getMovie(movList, i);
